...
server.listen(PORT, () => {
  console.log(`TagiTopia server :${PORT}`);
  // Keep-alive: ping self every 14 min so Railway/Render doesn't sleep
  const SELF_URL = process.env.RAILWAY_STATIC_URL || process.env.RENDER_EXTERNAL_URL;
  if (SELF_URL) {
    const https = require('https');
    setInterval(() => {
      https.get(`https://${SELF_URL}/health`, r => console.log('ping', r.statusCode)).on('error', ()=>{});
    }, 14 * 60 * 1000);
  }
});
